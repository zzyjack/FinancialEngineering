...
for (period = 1:N_periods)
   ...
   for(strategy =1:7)
      ...
      % Compute strategy
        [x{strategy,period} cash{strategy,period} weight{strategy,period} tran_cost{strategy,period} borrowed_fund{strategy,period}] = fh_array{strategy}(curr_positions, curr_cash, mu, Q, cur_prices);
      ...
      cash_deposit = 1;
          while cash{strategy,period} < 0
              ...
              x{strategy,period} = (cur_prices*curr_positions + borrowed_fund{strategy,period}-cash_deposit)*weight{strategy,period}./cur_prices';
              ...
              cash{strategy,period} = curr_cash +borrowed_fund{strategy,period}+ cur_prices*(curr_positions - x{strategy,period})-tran_cost{strategy,period};
      ...
      portf_value{strategy}(day_ind_start:day_ind_end) = data_prices(day_ind_start:day_ind_end,:) * x{strategy,period} + cash{strategy,period} - borrowed_fund{strategy,period};
      % Compute the shares after deducting the borrowed fund
       if strategy == 6
          x{strategy,period} = x{strategy,period} - borrowed_fund{strategy,period}.*weight{strategy,period}./data_prices(day_ind_end,:)';
       end
