...
function  [x_optimal cash_optimal w_optimal tran_cost borrowed_fund] = strat_max_Sharpe(x_init, cash_init, mu, Q, cur_prices)
    borrowed_fund = 0;
    ...
    cplex1.solve();
    if (strcmp(cplex1.Solution.statusstring,'infeasible') == 1)
        % if no solution can be obtained during financial crisis, keep the
        % portfolio unchanged
        x_optimal = x_init;
        cash_optimal = cash_init;
        w_optimal = (cur_prices'.*x_init)/(cur_prices*x_init);
        tran_cost =0;
